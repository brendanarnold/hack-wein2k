      SUBROUTINE DFXTPSS(fu,gmagu,tauup,fd,gmagd,taudn,gr,exu,exd)
...
!!        CALL FX_tpss(RHO2,S,2.d0*TAUup,EXU)
        CALL FX_tpss(RHO2,2.d0*gmagu,2.d0*TAUup,EXU)
      ELSE
       EXU=0.D0
      ENDIF
!! REPEAT FOR DOWN
...
!        S=2.d0*Gmagd/(2.D0*FK*RHO2)
        CALL FX_tpss(RHO2,2.d0*gmagd,2.d0*TAUdn,EXD)
      ELSE
       EXD=0.D0
      ENDIF
      RETURN
