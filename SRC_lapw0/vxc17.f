      SUBROUTINE vxc17(FU,GXU,GYU,GZU,GMAGU,G2U,GGXU,GGYU,GGZU, &
...
! EXCHANGE - UP
      ...
      U=GDGG/(TKF*(FU*TKF)**2)
      V=G2U/(FU*TKF**2)
      X=GMAGU/(FU**(1.33333333333D0))
      CALL EXCH17(D,S,U,V,X,EXUP,VXUP)
! EXCHANGE - DN
      D=2.D0*FD
      ...
      U=GDGG/(TKF*(FD*TKF)**2)
      V=G2D/(FD*TKF**2)
      X=GMAGD/(FD**(1.33333333333D0))
      CALL EXCH17(D,S,U,V,X,EXDN,VXDN)
! LOCAL CORRELATION
      D=FU+FD
      RS=(0.75D0/(PI*D))**THRD
      ZET=(FU-FD)/D
