      ...
      character*3 matrix
      character*10 titel,tit1,tit(0:1000)
...
!      open (unit=20,file='case.plt',form='formatted',status='unknown')
  ...
  70    read(10,140) tit1,akernz
        do 40 j=int,int-(mult(i)*faktor-1),-1
        kernz(j)=nint(akernz)
!           write(*,*) akernz,kernz(j)      
        rad(j)=a/10.d0
        tit(j)=tit1
  40    continue    
      read(10,*)
...
!      write (20,190) titel
!       write charge (=0 for neutral cell) and spin multiplicity (=1 for singlet state)
        write(20,'(i2,/)') int
      do 20 j=1,int
      ikern(j)=idint(kernz(j))
!      write(20,210) ikern(j),x(j)*0.529177,y(j)*0.529177,z(j)*0.529177 
      write(20,210) tit(j),x(j)*0.529177,y(j)*0.529177,z(j)*0.529177 
 210  format(a10,4f15.9)
 20   continue     
      theta=75.
      phi=-70.
      rot=0.
...
1010  if(i.gt.76)goto 9992
      ...
      open(unit=20,file='input',form='formatted',status='new',err=9994)
      return
9992  write(*,*)'Filename too long' 
      stop      
9993  write(*,*)'struct-File does not exist'      
      stop
9994  write(*,*)'File   input   already exists.'
      write(*,*)' Overwrite (y/n)?'
      read(*,99901)test
