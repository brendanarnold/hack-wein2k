!BOP
! !ROUTINE: AngularXSpectrum
! !INTERFACE:
      SUBROUTINE AngularXSpectrum (PP, QQ, EnergyOut, wi,ind)
! !USES:
      use constants
...
!         we start with the DOS contributions
	      indx=1+l*(l+1)+m
	      SDLM(indx,IPos,ind)=SDLM(indx,IPos,ind) + dble (Factor4)
! dipole-allowed L mixing
	    elseif (ABS(l-ll).EQ.2) then
	      if (ABS(m-mm).EQ.2) then		  ! 00 22
	        CTR(1,IPos,ind)=CTR(1,IPos,ind) + Factor4
	      else if (ABS(m-mm).EQ.1) then		  ! 00 21
	        CTR(2,IPos,ind)=CTR(2,IPos,ind) + Factor4
	      else if (ABS(m-mm).EQ.0) then		  ! 00 20
	        CTR(3,IPos,ind)=CTR(3,IPos,ind) + Factor4
...
! l'=L'=1
	      if (l.EQ.1) then
	        if (ABS(m-mm).EQ.2) then		! 11 1-1
	          CTR(4,IPos,ind)=CTR(4,IPos,ind) + Factor4
	        else if (ABS(m-mm).EQ.1) then		! 11 10
	          CTR(5,IPos,ind)=CTR(5,IPos,ind) + Factor4
	        end if  	! all possible m'M' combinations for l'=L'=1
! l'=L'=2
	      elseif (l.EQ.2) then
	        if (ABS(m-mm).EQ.4) then		! 22 2-2
	          CTR(6,IPos,ind)=CTR(6,IPos,ind) + Factor4
	        else if (ABS(m-mm).EQ.3) then		! 22 2-1
	          CTR(7,IPos,ind)=CTR(7,IPos,ind) + Factor4
	        else if (ABS(m-mm).EQ.2) then		! 21 2-1 or 22 20
	          if (ABS(m+mm).EQ.2) then		  ! 22 20
	            CTR(8,IPos,ind)=CTR(8,IPos,ind) + Factor4
	          else					  ! 21 2-1
	            CTR(9,IPos,ind)=CTR(9,IPos,ind) + Factor4
	          end if
	        else if (ABS(m-mm).EQ.1) then		! 22 21 or 21 20
	          if (ABS(m+mm).EQ.3) then		  ! 22 21
	            CTR(10,IPos,ind)=CTR(10,IPos,ind) + Factor4
	          else					  ! 21 20
	            CTR(11,IPos,ind)=CTR(11,IPos,ind) + Factor4
	          end if
	  ...
	  elseif(l.eq.3.and.ll.eq.3.and.m.eq.mm) then
	    sdlm(10,IPos,ind)=sdlm(10,IPos,ind) + Factor4
	  else   !  term is a CT with l=3 or ll=3
	    ctr(12,IPos,ind)=ctr(12,IPos,ind) + Factor4
...
!---------------------------------------------------------------------------	
                              ...
                              ENDDO !m
                           ENDDO !Mu
                           ENDIF ! LC+ll+LambdaPr must be even 
                        ENDDO !LambdaPr
                        ENDIF ! LC+l+Lambda must be even 
                     ENDDO !Lambda
