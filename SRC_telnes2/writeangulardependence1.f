...
! !USES:
    use input, only : npos, k0len, Energy
...
!EOP
      ...
      real*8 Q(npos),pi(npos),sigma(npos),tot(npos), &
      totdip(npos),sigmadip(npos),QVs(3,npos),theta(npos),mono(npos),quad(npos),octo(npos)
	  real*8 beta,k0local
      integer k
      ...
      do k=1,npos
        ...
        quad(k)=(sdlm(5,k,1)+sdlm(6,k,1)+sdlm(7,k,1)+sdlm(8,k,1)+sdlm(9,k,1))/weightv(k)
...
!		else
		    k0local=k0len
!		endif
        theta(k)=Q(k)*dsin(QVs(2,k))*dble(-1000)/ dsqrt((k0len**2+Q(k)**2-2.0*k0len*Q(k)*dcos(QVs(2,k))))
...
! This is the purpose of the variable k0local.
      ...
      if (headers) write(60,'(a)') '## theta  pi(1_0)  sigma(11 1-1 00)  total(**)  sigmadipole(11 1-1) ',&
              'totaldipole(1*) monopole(00) quadrupole(2*)'
      do k=1,npos
        write(60,10) theta(k),pi(k),sigma(k),tot(k),sigmadip(k),totdip(k),mono(k),quad(k),octo(k)
      enddo
10    format(9F17.11)
