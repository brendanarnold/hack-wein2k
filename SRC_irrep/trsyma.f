      SUBROUTINE TRSYMA(FL,ISK,ISKDEN,LKG,IKG,TAU,CNAM,SK,IORD, &
...
!
      ...
      DIMENSION        SK(3),ISK(3),TAU(3,NSYM)
...
!........sum over |X(i,Pg)-conj(X(j,Pg))|
         IWRT=IWRT+1
         ARG=-2*PI*DBLE( ISK(1)*TAU(1,LKG(IG))/ISKDEN &
                        +ISK(2)*TAU(2,LKG(IG))/ISKDEN &
                        +ISK(3)*TAU(3,LKG(IG))/ISKDEN )
