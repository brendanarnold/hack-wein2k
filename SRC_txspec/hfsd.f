!$hp9000_800 intrinsics on
      SUBROUTINE HFSD(NATOM,NPRIM,LPRIM,KAPPA,RELA) 
!                                                                       
...
!fe
!pi	
      INTEGER NATOM,NPRIM,LPRIM,KAPPA
!pie
      CHARACTER *10 BAR                                                 
...
!
!     NINI is initial value of NATT; number of atoms
!     used for checking which atom was selected
!
      NINI=NATT
...
! **********************************************************************
      ...
      DE=DEN(J)      
!
!     NQN..N; NQL..L; MK..kappa
!
!      WRITE(47,*) NINI-NATT+1,NQN(J),NQL(J),NK(J)
!
...
!
 ...
 24   CONTINUE
!     for testing:
!      write(47,4711) DR(I),DQ(I)
!
!     PI
!
!     Return to main if n,l and kappa match the
!     given criteria:
!
!     selection criteria are: natom (position of atom in struct file)
!                             n (nprim)
!                             l (lprim)
!                             k (kappa)
!
!     while jumping back to main, the COMMON of /DIRA/
!     containing the core _radials_ DP() and DQ() are
!     returned.
!      
      IF(NATOM.EQ.(NINI-NATT+1)) then
         if(NPRIM.EQ.NQN(J).AND.LPRIM.EQ.NQL(J).AND. &
              Kappa.EQ.NK(J)) THEN
              RETURN
         endif
      ENDIF
...
!
!     PI
!     if requested n,l and/or kappa were not matched:
!     reset NP to zero and exit gracefully!
         NP=0
         RETURN
...
!                                                                       
 ...
 2002 FORMAT(3X,'CORE STATES =', 20(I2,A2) )                            
