SUBROUTINE ROTDEF (iz,tau,iord,nat,pos,ndif,rotij,tauij,mult,lattic)
  ...
  REAL(8),INTENT(IN)  :: pos(nat*48,3),tau(3,iord)
  ...
  DO JATOM=1,NAT                                                 
     ...
     DO M=1,MULT(JATOM)                                          
        ...
        DO I=1,IORD                                              
           x(1:3)=0.d0
           x=MATMUL(TRANSPOSE(iz(1:3,1:3,i)),pos(index1,1:3))
           x(1:3)=x(1:3)+tau(1:3,i)
           x1(1:3)=MOD(ABS(X(1:3)-POS(INDEX,1:3))+toler,one)-toler
...
900 CALL OUTERR('ROTDEF','no symmetry operation found.')
  ...
  WRITE(ERRMSG,9010) (POS(index1,i1),I1=1,3) 
  CALL OUTERR('ROTDEF',ERRMSG)
  WRITE(ERRMSG,9020) (POS(INDEX,i1),I1=1,3) 
  CALL OUTERR('ROTDEF',ERRMSG)
  STOP 'ROTDEF - Error'
