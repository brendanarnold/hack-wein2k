...
!CC   Note: increase size of BINT, B
      INCLUDE 'param.inc'
      DIMENSION BCOF(NDIM,LM),  BINT(LMAX2+1,3,3),  F(2),  BFAC(2),  LL(2),    &
         FI(3),  B(LMAX2+1),  BDIF(3)                                         
...
!CC        IF H*P .LT. EPS4, SIMPSON IS USED.                           
      ...
      XEPS = 0.d0                                                        
      IF (L1 .GT. 0) XEPS = (EPS*DFLM)**(1.d0/FLOAT(L1))                  
...
!CC DOWNWARD RECURSION. USE SERIES EXPANSION FOR STARTING VALUES        
   ...
   10 BFAC(I) = 1.d0                                                      
      DO 15 I2=1, 3                                                     
   15 FI(I2) = 1.d0/(LL(1)+I2)                                            
!CC SERIES EVALUATION                                                   
      IND = 0                                                           
   20 IND = IND+1                                                       
      DO 30 I=1, 2                                                      
      BFAC(I) = -BFAC(I)/(4.d0*IND*(LL(I)+IND+.5d0))*XX                     
   30 F(I) = F(I)+BFAC(I)                                               
      DO 35 I2=1, 3                                                     
...
!CC LARGE ARGUMENT CASE, UPWARD RECURSION                               
  100 S = SIN(X)                                                        
      ...
      BINT(1,2,M) = (-C+1.d0)*XFAC                                        
      BINT(1,3,M) =(S-X*C)*XFAC                                         
      IF (LM.EQ.1) GOTO 200                                             
      BINT(2,1,M) = -B(1)+XFAC                                          
      BINT(2,2,M) = BINT(1,1,M)-S*XFAC                                  
      BINT(2,3,M) =(-2.d0*C-X*S+2.d0)*XFAC                                  
      IF (LM.EQ.2) GOTO 200                                             
!CC                                                                     
      DO 110 L=3, LM                                                    
...
!CC CONSTRUCT BCOF ARRAY                                                
  ...
  210 BDIF(I2) = BINT(L,I2,M)-BINT(L,I2,M1)                             
      BCOF(2*IN-2,L) = -BDIF(3) + 2.d0*X1*BDIF(2)                          &
       - (X1*X1-DD)*BDIF(1)                                             
      IF (IN.GT.2) GOTO 240                                             
      BCOF(1,L) = .5d0*BDIF(3) - (X2+1.5d0*D)*BDIF(2)                        &
       + ((.5d0*X2+1.5d0*D)*X2+DD)*BDIF(1)                                  
      GOTO 230                                                          
  240 BCOF(2*IN-3,L) = .5d0*(BINT(L,3,M)-BINT(L,3,M2))                     &
       -X2*(BINT(L,2,M)-BINT(L,2,M2))                                    &
       -1.5d0*D*(BINT(L,2,M)-2.d0*BINT(L,2,M1)+BINT(L,2,M2))                 &
       +(.5d0*X2*X2+DD)*(BINT(L,1,M)-BINT(L,1,M2))                         &
       +1.5d0*X2*D*(BINT(L,1,M)-2.d0*BINT(L,1,M1)+BINT(L,1,M2))             
      IF (IN.EQ.N) BCOF(NPOINT,L) = .5d0*BDIF(3) - (X-1.5d0*D)               &
       *BDIF(2) + ((.5d0*X-1.5d0*D)*X+DD)*BDIF(1)                           
  230 CONTINUE                                                          
!CC                                                                     
!CC                                                                     
    5 X = X+D2                                                          
...
!CC                                                                     
  410 DO 420 L=1, LM                                                    
      BCOF(1,L) = .5d0*BCOF(1,L)                                          
  420 BCOF(NPOINT,L) = .5d0*BCOF(NPOINT,L)                                
      RETURN                                                            
      END                                                               
