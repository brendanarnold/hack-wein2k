...
!....read bandenergy correction
 ...
 9    READ(21,700,END=8) MARGN
        IF(MARGN(10:19).EQ.'BANDENERGY') THEN
            READ(MARGN,'(31x,f12.8)') ts2
            write(6,*) '      BANDENERGY CORRECTION:',ts2
            goto 8
        endif
        ...
        IF(MARGN(11:16).EQ.'-(T*S)') THEN
            READ(MARGN,'(30x,f12.8)') ts2
            write(6,*) '      ENTROPY CORRECTION:',ts2
            goto 8
        endif
       goto 9
 8    continue
 ...
 10      READ(21,700,END=20) MARGN
        ...
        IF(MARGN(2:4).EQ.'CHA') THEN
         ...
         ELSE IF(MARGN(4:8).EQ.'QTL-B') THEN
            READ(MARGN,1001)HELP,help1,jhelp1,jhelp2
            IF(HELP.GT.QTLB) then 
             QTLB=HELP
             qtlbe=help1
             jqtlbe=jhelp1
             lqtlbe=jhelp2
            endif
            WRITE(6,*)'QTL-B=',QTLB,'energy',qtlbe,'jatom',jqtlbe,' l=',lqtlbe
         ELSE
            IF(ILOOP.EQ.IPROC)THEN
!               call strln(MARGN,LEN1)
               len=len_trim(margn)
!               print*, 'len',len,len1
               IF(LEN.LT.1) LEN=1 
 ...
 20      CONTINUE
      ...
      IF(QTLB.NE.0)THEN
         WRITE(22,'(//)')
         WRITE(22,1000)QTLB,qtlbe,jqtlbe,lqtlbe
      ENDIF
...
!
 ...
 730  FORMAT(':SUM  :',1X,'SUM OF EIGENVALUES =  ',F20.9)
 731  FORMAT(7X,1X,22x,F20.9)
 880  FORMAT(':CHA  :',' TOTAL CHARGE INSIDE UNIT CELL =',F15.6) 
 881  FORMAT(7x,32x,F15.6)
 1000 FORMAT('   QTL-B VALUE .EQ. ',F10.5,'   in Band of energy ',F10.5,&
                 3x,'ATOM=',i5,3x,'L=',i3,/)
 1001 FORMAT(20X,F10.5,21x,f10.5,8x,i5,5x,i3)
