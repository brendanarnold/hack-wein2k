...
!fe
!                                                                       
!.....READ TAPE22=FERMI-ENERGY OF CASE:SCF2 FOR V_RESP
!                                                                       
!!      READ(22,700,END=7)  RMARGN,EFERM                                 
! 7    CONTINUE                                                          
!      WRITE(6,*)'FERMI-ENERGY:',EFERM                                         
! 700  FORMAT(//////,A39,f9.5)                                        
!                                                                       
      WRITE(9,1960) ISCF                                                
      WRITE(9,*) '    NORM: RHO*R*R/SQRT(4.*PI)  '         
      WRITE(9,*)                                                        
!_vresp
      WRITE(28,1960) ISCF                                                
      WRITE(28,*) '    NORM: RHO*R*R/SQRT(4.*PI)  '         
      WRITE(28,*)                                                        
!_vresp
!                                                                       
!                                                                       
!                                                                       
  1   NSTOP=1                                                           
      CALL INSLD(RELA,npxx,cform,shift)                                            
! ....core wavefunctions onto unit 29
      write(29,*) norb,' core states for this atom'
...
! **********************************************************************
 ...
 23   DGC(I,J)=DP(I)                                                    
      DPC(I,J)=DQ(I)                                                    
!v_resp
      vD(I)=vD(I)+(NEL(J)*(DP(I)*DP(I)+DQ(I)*DQ(I))) &
      *(-2.0d0*den(j))
      if (i.eq.1) write (6,*) 'eigenwert von ',j, 'Zustand',den(j)
!v_resp
 24   D(I)=D(I)+NEL(J)*(DP(I)*DP(I)+DQ(I)*DQ(I))                        
      WRITE(29,2002)  NQN(J),TITRE(J)        
 ...
 33   CONTINUE                                                          
      ...
      IF(TETS.LE.TEST.AND.EMAX.LE.TESTE.AND.VMAX.LE.TESTV.AND.YMAX.LE.TESTY) GO TO 61                                                     
      ITER=ITER+1                                                       
...
44    REWIND 10                                                         
 ...
 45   DO 55  I=1,NP                                                     
      ...
      DVN(I)=DV(I)                                                      
      DVF(I)=DC(I)                                                      
 55   DV(I)=DVAL*DV(I)+(1.d0-DVAL)*DC(I)                                  
      IF (.NOT.OLD) GO TO 56                                            
      READ(10,END=551) DV                                               
551   OLD=.FALSE.                                                       
      GO TO 4                                                           
...
!     IF (I.LE.0) GO TO 63                                              
 4000 JATOM=JATOM+1                                                     
      ...
      WRITE(9,2031)                                                     
!_vresp
      WRITE(28,1990) JATOM,BAR                                           
      WRITE(28,2001) 1                                                   
      WRITE(28,2002) ( NQN(JSTATE),TITRE(JSTATE), JSTATE=1,NORB )        
      WRITE(28,1980)                                                     
      WRITE(28,2011) 0,0                                                 
      NP40=NP-npxx                                                        
      WRITE(28,iform1) ( VD(KK), KK=1,NP40)                                 
      WRITE(28,2030)                                                     
      WRITE(28,2031)                                                     
!_vresp
      WRITE (6,1005)                                                    
...
! **********************************************************************
 ...
 99   CALL SOMM (DR,DP,DQ,DPAS,DC(1),0,NP)                              
      DO 105  I=1,NP                                                    
      DP(I)=D(I)*DVF(I)                                                 
 105  D(I)=D(I)*((D(I)*DR(I))**(1.d0/3.d0))                                 
      DC(2)=3.d0                                                           
      DC(3)=1.d0                                                           
...
! **********************************************************************
      DO I=2,NORB                                                  
         K=I-1                                                             
         DO J=1,K                                                     
            IF(NQL(I).NE.NQL(J).OR.NK(I).NE.NK(J)) CYCLE
            IM=NMAX(J)                                                        
...
!
 ...
 961  INFO = 8
!
!        Error due to positive eigenvalue
!
      WRITE (ERRMSG,9041) NSTOP,NQN(J),TITRE(J),jatom,bar
      CALL OUTERR('CORE',ERRMSG)
      WRITE (ERRMSG,9042) 
      CALL OUTERR('CORE',ERRMSG)
      GOTO 999
...
!                                                                       
 710  FORMAT(//,5X,'CORE STATES CONTRIBUTION TO TOTAL ENERGY',           &
             '   CALCULATED BY CORE',/)                                 
 720  FORMAT(/6X,I3,'.ATOM ',5X,A10,11X,I2,' CORE STATES')              
 730  FORMAT(':',I1,A2,i3.3,': ',i1,a2,9X,F20.9,' Ry') 
 1000 FORMAT(L7)                                                        
 ...
 9041 FORMAT('NSTOP=',i4,' positive eigenvalue for',I3,A2,' Atom:',i4,1x,a10)
 9042 FORMAT('Try to apply a potential shift in case.inc')
      END                                                               
