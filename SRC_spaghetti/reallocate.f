	module reallocate
	  ...
	  interface doreallocate
	    ...
	    module procedure doreallocate_ca12_d1
!	    module procedure doreallocate_ca3_d1
	    module procedure doreallocate_l4_d1
	...
	contains
	  ...
	  subroutine doreallocate_ca12_d1(tf, newdimension)
	    character*12, pointer :: hilfsfeld(:), tf(:)
	    allocate(hilfsfeld(newdimension))
            min1=min(newdimension,size(tf,1))
	    hilfsfeld(1:min1)=tf(1:min1)
	    deallocate(tf)
...
!	  subroutine doreallocate_ca3_d1(tf, newdimension)
...
!	  end subroutine 
	  ...
	  subroutine doreallocate_l4_d1(tf, newdimension)
	    logical, pointer :: hilfsfeld(:), tf(:)
	    allocate(hilfsfeld(newdimension))
            min1=min(newdimension,size(tf,1))
	    hilfsfeld(1:min1)=tf(1:min1)
	    deallocate(tf)
	  ...
	  subroutine doreallocate_i4_d3(tf, newdimension1, newdimension2, newdimension3)
	    integer*4, pointer :: hilfsfeld(:,:,:), tf(:,:,:)
