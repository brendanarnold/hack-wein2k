       subroutine zher2m (uplo,transa,transb,m,n, &
...
!
	 ...
	 do ii=1,m,nblock
	   ...
	   do j=ii,imax
	     call zgemv('n', imax-j+1, n, alpha, a(j,1), lda, &
                        b(j,1), ldb, beta, c(j,j), 1)
	   end do
           if(imax.lt.m) &
             call zgemm('n',transb,m-imax,imax-ii+1,n, &
	 ...
	 do ii=1,m,nblock
	   imax=min(m,ii+nblock-1)
	   do j=ii,imax
	     call zgemv(transa, n, imax-j+1, alpha, a(1,j), lda, &
                        b(1,j), 1, beta, c(j,j), 1)
	   end do
           if(imax.lt.m) &
	     call zgemm(transa,'n',m-imax,imax-ii+1,n, &
	   ...
	   do j=jj,jmax
	     call zgemv(transa, n, j-jj+1, alpha, a(1,jj), lda, &
                        b(1,j), 1, beta, c(jj,j), 1)
	   end do
           if(jj.ne.1) &
             call zgemm(transa,'n',jj-1,jmax-jj+1,n, &
                    alpha,a(1,1),lda,b(1,jj),ldb, &
                    beta,c(1,jj),ldc)
	 end do
       else
