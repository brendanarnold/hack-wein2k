      SUBROUTINE ZHEEVX2( JOBZ, RANGE, UPLO, N, A, LDA, VL, VU, IL, IU, &
                         ABSTOL, M, W, Z, LDZ, WORK, LWORK, RWORK, &
                         IWORK, IFAIL, BLOCK, NUME, NBVL, INFO )
      IMPLICIT NONE
!
...
!  =======
!
!  ZHEEVX2 computes selected eigenvalues and, optionally, eigenvectors
...
!          where NB is the blocksize for ZHETRD2 returned by ILAENV.
!
...
!
!  BLOCK   (input)  INTEGER.
!          On entry, BLOCK specifies the algorithmic block size.
!          BLOCK must be at least one.
!          Unchanged on exit.
!
!  NUME    (input) INTEGER
...
!     .. External Subroutines ..
      EXTERNAL           DCOPY, DSCAL, DSTEBZ2, DSTERF, XERBLA, ZDSCAL, &
                         ZHETRD2, ZLACPY, ZSTEIN, ZSTEQR, ZSWAP, ZUNGTR, &
                         ZUNMTR
!     ..
!     .. Intrinsic Functions ..
...
!
   ...
   20       CONTINUE
      ...
      END IF
!
!     Call ZHETRD2 to reduce Hermitian matrix to tridiagonal form.
!
      INDD = 1
      INDE = INDD + N
      INDRWK = INDE + N
      ...
      CALL ZHETRD2( UPLO, N, A, LDA, RWORK( INDD ), RWORK( INDE ), &
                   WORK( INDTAU ), WORK( INDWRK ), LLWORK, BLOCK,  &
                   IINFO )
      LOPT = N + WORK( INDWRK )
!
