...
!
      ...
      EI = EI*0.5D+0
      DE = DE*0.5D+0
      CALL OUTWIN(REL,VR,RNOT,DX,JRI,EI,FL,U,DU,NODE,ZZ)
      EIUP = EI
      EIDN = EI
      E1 = -100.0D+0
      E2 = -100.0D+0
   ...
   10 CONTINUE
      EIUP = EIUP + DE
      CALL OUTWIN(REL,VR,RNOT,DX,JRI,EIUP,FL,UUP,DUUP,NODE,ZZ)
      UTST = UPOLD*UUP
      DUTST = DUPOLD*DUUP
...
!   99 FORMAT(' L,EIUP,UTST,DUTST',I2,F8.4,2E12.3)
   ...
   20    CONTINUE
         EIDN = EIDN - DE
         CALL OUTWIN(REL,VR,RNOT,DX,JRI,EIDN,FL,UDN,DUDN,NODE,ZZ)
         UTST = DNOLD*UDN
         DUTST = DDNOLD*DUDN
...
!
         ...
         ELSE
            EI = 2.0D+0
            IF(myid.eq.0) WRITE (6,6000)  L, EI, E1, E2
            IF(myid.eq.0) WRITE (21,6000) L, EI, E1, E2
         ENDIF
