      SUBROUTINE LMSORT(NATO,NGAUNT,JATOM,LQIND,LQNS,GFAC,L0PTR, &
...
!        Arguments
!
      INTEGER            NATO, NGAUNT, JATOM
      INTEGER            L0PTR(NGAUNT+2),  L0FLD(NGAUNT)
      INTEGER            M0PTR(NGAUNT+2),  M0FLD(NGAUNT)
      INTEGER            LPPTR(NGAUNT+2),  LPFLD(NGAUNT)
      INTEGER            MPPTR(NGAUNT+2),  MPFLD(NGAUNT)
      INTEGER            LMXPTR(NGAUNT+2), LMXFLD(NGAUNT)
      INTEGER            LQIND(NATO)
      INTEGER            LQNS(6,NGAUNT,NATO)
      COMPLEX*16         GFAC(NGAUNT,NATO), GNTFLD(NGAUNT)
!
!     ..................................................................
!
...
!
      ...
      DO 30 I = 2, LQIND(JATOM)
!
!        Sort next L0, M0, LP, MP, LM tupel into existing fields
!
         L0 = LQNS(3,I,JATOM) - 1
         M0 = LQNS(6,I,JATOM)
         LP = LQNS(1,I,JATOM) - 1
         ...
         LMX = LQNS(4,I,JATOM)
         DO 10 IDX = I - 1, 1, -1
            IF ((L0 .LT. L0FLD(IDX)) .OR. &
                (L0 .EQ. L0FLD(IDX)).AND.((M0 .LT. M0FLD(IDX)) .OR. &
                (M0 .EQ. M0FLD(IDX)).AND.((LP .LT. LPFLD(IDX)) .OR. &
                (LP .EQ. LPFLD(IDX)).AND.((MP .LT. MPFLD(IDX)) .OR. &
                (MP .EQ. MPFLD(IDX)).AND.(LMX .LT. LMXFLD(IDX)))))) THEN
...
!
      PTR = 1
      DO 60 I = 1, LQIND(JATOM)
         IF (M0PTR(I+1) .EQ. NIL) THEN
            GOTO 70
         ENDIF
         M0 = M0FLD(M0PTR(I))
         M0FLD(PTR) = M0
         LPPTR(PTR) = M0PTR(I)
         PTRBEG = PTR
         DO 50 IDX = M0PTR(I) + 1, M0PTR(I+1) - 1
...
!
      PTR = 1
      DO 90 I = 1, LQIND(JATOM)
         IF (LPPTR(I+1) .EQ. NIL) THEN
            GOTO 100
         ENDIF
         LP = LPFLD(LPPTR(I))
         LPFLD(PTR) = LP
         MPPTR(PTR) = LPPTR(I)
         PTRBEG = PTR
         DO 80 IDX = LPPTR(I) + 1, LPPTR(I+1) - 1
...
!
      PTR = 1
      DO 120 I = 1, LQIND(JATOM)
         IF (MPPTR(I+1) .EQ. NIL) THEN
            GOTO 130
         ENDIF
         MP = MPFLD(MPPTR(I))
         MPFLD(PTR) = MP
         LMXPTR(PTR) = MPPTR(I)
         PTRBEG = PTR
         DO 110 IDX = MPPTR(I) + 1, MPPTR(I+1) - 1
...
!
      PTR = 1
      DO 150 I = 1, LQIND(JATOM)
         IF (LMXPTR(I+1) .EQ. NIL) THEN
            GOTO 160
         ENDIF
         LMX = LMXFLD(LMXPTR(I))
         LMXFLD(PTR) = LMX
         ...
         PTRBEG = PTR
         DO 140 IDX = LMXPTR(I) + 1, LMXPTR(I+1) - 1
