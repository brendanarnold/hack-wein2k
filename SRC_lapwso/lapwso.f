...
!
 ...
 2030 format(///)
      ...
      ALLOCATE(p(labc+1,nato,2),dp(labc+1,nato,2),pe(labc+1,nato,2), &
               dpe(labc+1,nato,2))
      ...
      call garadme(e,vru,p,dp,pe,dpe,ri_mat, &
                   jspin,kpot,ipr)
	...
	do ity=1,nat
         do l=1,lmx
          if(.not.lapw(l-1,ity)) &
          e(l,ity,isi)=e(l,ity,isi)+200.d0
         enddo  
 ...
 100  kkk = kkk + 1
      do isi=1,jspin
         call cputim(dtime0)
         call kptin(flcom,meigve,ss,ne(isi),nv(isi),ee,kv,bname, &
                   weight,kkk,isi)
         call cputim(dtime1)
         cp(1)=cp(1)+dtime1-dtime0
         if (kkk.lt.0) goto 101
         call cputim(dtime0)
      ...
      if(.not.fl) then
        ...
        nv(2)=0
         ...
         call cputim(dtime0)	
      call hmsec(fl,emm,ne,nv,ee,meigve, &
                 ri_mat,ri_orb,kkk,jspin,ipr)
         call cputim(dtime1)
         cp(3)=cp(3)+dtime1-dtime0
         call cputim(dtime0)
      call kptout(ss,bname,weight,kkk,kv, &
                  jspin,nv,ne)
         call cputim(dtime1)
