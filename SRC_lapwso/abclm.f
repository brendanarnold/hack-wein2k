      SUBROUTINE ABCLM(meigve,SS,NE,NV,KV,P,DP,PE,DPE, &
                       isi)   
      USE param
...
!      complex*16     yl(labc2),h_yl(labc2,hblock), &
!                     h_alyl(labc2,hblock),h_blyl(labc2,hblock), &
!                     alm(labc2,nume),blm(labc2,nume)
      complex*16,allocatable ::  yl(:),h_yl(:,:), &
                     h_alyl(:,:),h_blyl(:,:), &
                     alm(:,:),blm(:,:)
      ...
      DIMENSION      P(Labc+1,NATO,2),DP(Labc+1,NATO,2),PE(Labc+1,NATO,2), &
                     DPE(Labc+1,NATO,2)
      DIMENSION      SS(3),KV(3,NMAT)

      DATA           CZERO/(0.0D0,0.0D0)/,IMAG/(0.0D0,1.0D0)/
      DATA           ZERO/0.0D0/
      DATA           TOL1/1.0D-6/
 
      allocate     ( yl(labc2),h_yl(labc2,hblock), &
                     h_alyl(labc2,hblock),h_blyl(labc2,hblock), &
                     alm(labc2,nume),blm(labc2,nume) )
      PI=ACOS(-1.0D0)                                                   
      TWOPI=2.D0*PI                                                     
	...
	call cputim(dt0)
      ...
      DO 120 II=1,NV-(nlo(1)+nlon(1)+nlov(1)),hblock
      i3=0
      DO 121 I=ii,min(ii+hblock-1,NV-(nlo(1)+nlon(1)+nlov(1)))
      i3=i3+1
        BK(1)=BKX(I)                                                      
        BK(2)=BKY(I)                                                      
        BK(3)=BKZ(I) 
 ...
 121    CONTINUE
         ...
         enddo
           ...
           call zgemm('N','N',index,ne,ibb,(1.d0,0.d0), &
           h_alyl,lda,meigve(ii,1,isi),ldb,(1.d0,0.d0), &
           alm,lda)
           call zgemm('N','N',index,ne,ibb,(1.d0,0.d0), &
           h_blyl,lda,meigve(ii,1,isi),ldb,(1.d0,0.d0), &
           blm,lda)
  120 CONTINUE
      index=0
       do l=0,LABC
      ...
      enddo
      if (nlo(ja).ne.0) call  &
        lomain(meigve,bkx,bky,bkz,fac,ne,lfirst,indj,JA,nv,isi)
      if (nrlo(ja).ne.0) call &
        rlomain(bkx,bky,bkz,fac,ne,lfirst,indj,JA,nv,isi,kv)
